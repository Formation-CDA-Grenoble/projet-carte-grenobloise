 ...
 * @author Vaadin Ltd
 */
public class ButtonSearch extends GeneratedVaadinButton<ButtonSearch> implements HasSize, HasEnabledBuilder {

    private ComponentJoin iconComponent;
    private boolean iconAfterText;
    private boolean disableOnClick = false;
    private boolean disableOnClickConfigured = false;

    // Register immediately as first listener
    private Registration disableListener = addClickListener(buttonClickEvent -> {
        if (disableOnClick) {
            doDisableOnClick();
        }
    });
     ...
     *
     * @param search the text inside the button
     * @see #setText(String)
     */
    public ButtonSearch(String search) {
        setText(search);
    }
     ...
     */
    public ButtonSearch(Component icon) {
        setIcon(icon);
    }
     ...
     * @see #setIcon(Component)
     */
    public ButtonSearch(String text, Component icon) {
        setIcon(icon);
        setText(text);
    }
     ...
     *
     * @param text          the text inside the button
     * @param clickListener the event listener for click events
     * @see #setText(String)
     * @see #addClickListener(ComponentEventListener)
     */
    public ButtonSearch(String text, ComponentEventListener<ClickEvent<ButtonSearch>> clickListener) {
        setText(text);
     ...
     * @see #addClickListener(ComponentEventListener)
     */
    public ButtonSearch(Component icon, ComponentEventListener<ClickEvent<ButtonSearch>> clickListener) {
        setIcon(icon);
        addClickListener(clickListener);
    }
     ...
     * @see #addClickListener(ComponentEventListener)
     */
    public ButtonSearch(String text, Component icon, ComponentEventListener<ClickEvent<ButtonSearch>> clickListener) {
        setIcon(icon);
        setText(text);
        addClickListener(clickListener);
    }
     ...
     * <p>
     * This method removes any existing text-content and replaces it with the given
     * text.
     * <p>
     ...
     * operation or not.
     *
     * @param text the text content to set, may be <code>null</code> to only remove
     *             existing text
     */
    @Override
     ...
     * <p>
     * Even though you can use almost any component as an icon, some good options
     * are {@code Icon} and {@link Image}.
     * <p>
     * Use {@link #setIconAfterText(boolean)} to change the icon's position relative
     ...
     * operation or not.
     *
     * @param icon component to be used as an icon, may be <code>null</code> to only
     *             remove the current icon, can't be a text-node
     */
     ...
     *
     * @return the icon of this button, or <code>null</code> if the icon is not set
     */
    ...
    /**
     * Sets whether this button's icon should be positioned after it's text content
     * or the other way around.
     * <p>
     * At the element-level, this method determines whether to set
     * {@code slot="prefix"} or {@code slot="suffix"} attribute to the icon.
     *
     * @param iconAfterText whether the icon should be positioned after the text
     *                      content or not
     */
    ...
    /**
     * Gets whether this button's icon is positioned after it's text content or the
     * other way around.
     *
     * @return <code>true</code> if this button positions it's icon after it's text
     ...
     * {@link #setText(String)} and {@link #setIcon(Component)}, may have unexpected
     * results, e.g. in the order of child elements and the result of
     * {@link #getText()}.
     *
     ...
     * <p>
     * This property is not synchronized automatically from the client side, so the
     * returned value may not be the same as in client side.
     *
     ...
     *
     * @param disableOnClick true to disable button immediately when clicked
     */
    ...
    /**
     * Initialize client side disabling so disabled if immediate on click even if
     * server-side handling takes some time.
     */
    private void initDisableOnClick() {
        if (!disableOnClickConfigured) {
            getElement().executeJs("var disableEvent = function () {" + "if($0.getAttribute('disableOnClick')){"
    ...
    /**
     * Removes all contents from this component except elements in {@code exclusion}
     * array. This includes child components, text content as well as child elements
     * that have been added directly to this component using the {@link Element}
     * API.
     *
     * @see ButtonSearch#removeAll()
     */
